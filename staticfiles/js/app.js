...
(function ($) {
	...
	THEME.shopify = {
		...
		collectionLoaderCheck: function collectionLoaderCheck() {
		...
		shopifyBlog: function shopifyBlog() {
	...
	THEME.initialization = {
		...
		animations: function animations() {
			...
			var $animCollection = $('.collection-item.zoom-out');
		...
		newsletterModal: function newsletterModal(modal, checkbox) {
			...
			var $newsletter = $(modal),
			    $checkBox = $(checkbox);
			if ($newsletter.attr('data-only-index') == 'true' && !$('body[class*="template-index"]').length && !$('body[class*="page-index"]').length) {
				return false;
			}
			...
			var ProductPageGallery = {
				...
				_handlers: function _handlers() {
					...
					if ($previews.length) {
						...
						$document.on('click', '.product-previews-carousel a', function (e) {
							var $this = $(this);
							if ($this.hasClass('active')) {
								return false;
							} else {
	...
	THEME.productPage = {
		...
		countdownCircle: function countdownCircle() {
			...
			if ($('[data-date]').length && $('[data-locale]') != 'en') {
				var dateArray = $('[data-date]').html().split(',');
				var day = dateArray[0];
				switch (day) {
					case 'Mon':
	...
	THEME.catalog = {
		...
		ajaxShowMore: function ajaxShowMore(link) {
			var $link = $(link),
			    $container = $('.prd-grid'),
	...
	$document.on('shopify:section:select shopify:section:load', function () {
		...
		if ($('.bnslider').length) {
			$('.bnslider').each(function () {
				THEME.initialization.mainSlider("#" + $(this).attr('id'));
			});
		}
